...
public class DateUtils {
     ...
     * @return
     */
    public static Date todayClock(int hour){
        if (hour < 0 || hour > 23){
            throw new RuntimeException("获取时间出错");
        }
        LocalDateTime now = LocalDateTime.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern(DEFAULT_DATE);
        String dateStr = now.format(formatter);
        if (hour < 10){
            dateStr = dateStr + " 0" + hour + ":00:00";
        }else {
            dateStr = dateStr + " " + hour + ":00:00";
        }
        return Date.from(LocalDateTime.parse(dateStr, DateTimeFormatter.ofPattern(DEFAULT_DATE_TIME)).toInstant(ZoneOffset.of("+8")));
    }

    public static Date futureTime(Date date, long future, TimeEnum timeEnum){
        LocalDateTime localDateTime = LocalDateTime.ofInstant(date.toInstant(), DEFAULT_ZONE_OFFSET);
        switch (timeEnum){
            case YEAR:
                return Date.from(localDateTime.plusYears(future).toInstant(DEFAULT_ZONE_OFFSET));
            case DAY:
                return Date.from(localDateTime.plusDays(future).toInstant(DEFAULT_ZONE_OFFSET));
            case HOUR:
                return Date.from(localDateTime.plusHours(future).toInstant(DEFAULT_ZONE_OFFSET));
            case MINUTE:
                return Date.from(localDateTime.plusMinutes(future).toInstant(DEFAULT_ZONE_OFFSET));
            case SECOND:
                return Date.from(localDateTime.plusSeconds(future).toInstant(DEFAULT_ZONE_OFFSET));
            default:
                return date;
        }
    }
